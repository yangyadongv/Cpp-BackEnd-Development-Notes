...
- 辅助理解：**核心态**（又称为系统态），在此状态下，**能执行一切指令，访问所有的寄存器内容及存区域**（用一句不严谨的话说，啥都能干。） **用户态**，是相对核心态而言的，在此状态下**，只能执行特定的一些指令，特定的一些寄存器和一些特定的区域**。 那么问题来了，为什么要区分用户态和核心态呢？原因很简单，就是为了**保护操作系统**，因为操作系统中的一些重要数据，比如PCB，是不允许修改和破坏的。
